        ...
        <Box sx={style} className={'user_popup_section'}>
            <div className='d_flex justify_content_center'>
                <Typography id="modal-modal-title" variant="h6" component="h2" className={"block-user-title"}>
                    {
                        `${(modalValue?.isUserWithdraw || modalValue?.isPublisherStatus) ? 'Do you want to approve this request?' : 'Do you want to reject the game?'}`
                    }
            ...
            <div className='publisher_popup'>
                <form onSubmit={(e) => handleSubmit(e)}>
                    <div className={'textarea_form_field'}>
                        <textarea rows={4}
                            placeholder={`${(modalValue?.isPublisherStatus) ? 'Please input Transaction ID' : 'Reason for Rejecting Request'}`}
