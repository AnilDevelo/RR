...
import {clearPaginationAndFilterData,currencyFormat,renderSrNo} from "../../../../utils";
...
const GstTransactionHistory = ({ modalValue, handleOpenModal,setDistTab }) => {
  ...
  const getMGPWalletsHistoryDetails = (startDate, endDate) => {
    let payload = {
      ...
      startDate: startDate ? moment(startDate).format("YYYY-MM-DD") : null, endDate: endDate ? moment(endDate).format("YYYY-MM-DD") : null,
      exportFile: filterData?.exportFile,
      csvDownload: filterData?.csvDownload,
        searchText: filterData?.statusField === "All Transaction Type" ? "" : filterData?.statusField,
    };
        ...
        if (res?.data?.data?.filePath) {
          ...
          setRowData({
            ...
            TotalWallet: {
             ...
             totalTransactionTypeWise: res?.data?.data?.totalTransactionTypeWise,
            },
          });
        }
      }
  ...
  return (
    <>
       <div className={"tds-all-user-section"}>
        <Paper sx={{ mb: 2 }} className="outer-box">
          <div className={"tds-main-section"}>
            <button className={"tds-back-btn"}
              onClick={() => setDistTab(0)}
            >
          ...
          >
            {currencyFormat(rowData?.TotalWallet?.totalTransactionTypeWise)}
          </p>
              </div>
          </div>
        </Paper>
      ...
      <MainCommonFilter
          ...
          addPropsFilter={{ isPlatform: true, userPayment: rowData?.list?.length <= 0, }}
      ...
      <CommonModal className={'Approved-reject-section'} modalIsOpen={modalDetails.modalIsOpen} handleOpenModal={handleOpenModal}>
            <Modal modalValue={modalDetails.modalValue} handleOpenModal={handleOpenModalExport} modalIsOpen={modalDetails.modalIsOpen}  />
      </CommonModal>
    </>
  );
};
export default GstTransactionHistory;
