...
import { createTournamentLeaderboardRules, getTournamentLeaderboardRules } from "Redux/Tournament/action";
...
const LeaderboardRules = () => {
  ...
  const getLeaderboardRules = () => {
    setLoader(true);
    dispatch(getTournamentLeaderboardRules({})).then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (simpleValidator.current.allValid()) {
      ...
      dispatch(createTournamentLeaderboardRules(payload)).then((res) => {
    ...
    <>
      ...
      <Paper sx={{ mb: 2 }} className="outer-box game-rules-section">
        ...
        {hideActionFunc("tournament") && (
          ...
          {
            checkRchEmpty() ?
            <button className={'btn mt_2'} type={"submit"} onClick={(e) => handleSubmit(e)}>Save Leaderboard Rules</button>
              :
            <button className={'btn mt_2'} type={"submit"} onClick={(e) => handleSubmit(e)}>Update Leaderboard Rules</button>
          }
          </div>
        )}
      </Paper>
      ...
      >
        <Modal
          ...
          handleOpenModal={handleOpenModal}
          modalIsOpen={modalDetails.modalIsOpen}
          redirectApiHandler={getLeaderboardRules}
        />
      </CommonModal>
    </>
  );
