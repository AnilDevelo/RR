...
import { createRefundRules, getRefundRules } from "Redux/Tournament/action";
...
const RefundRules = () => {
  ...
  const getRefundRulesData = () => {
    setLoader(true);
    dispatch(getRefundRules({})).then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (simpleValidator.current.allValid()) {
      ...
      dispatch(createRefundRules(payload)).then((res) => {
    ...
    <>
      ...
      <Paper sx={{ mb: 2 }} className="outer-box game-rules-section">
        ...
        {hideActionFunc("tournament") && (
          ...
          {
            checkRchEmpty() ?
            <button className={'btn mt_2'} type={"submit"} onClick={(e) => handleSubmit(e)}>Save Refund Rules</button>
              :
            <button className={'btn mt_2'} type={"submit"} onClick={(e) => handleSubmit(e)}>Update Refund Rules</button>
          }
          </div>
        )}
      </Paper>
      ...
      >
        <Modal
          ...
          handleOpenModal={handleOpenModal}
          modalIsOpen={modalDetails.modalIsOpen}
          redirectApiHandler={getRefundRulesData}
        />
      </CommonModal>
    </>
  );
