...
import { createTournamentRules, getTournamentRules } from "Redux/Tournament/action";
...
const TournamentRules = () => {
  ...
  useEffect(() => {
    getTournamentRulesData();
  }, [dispatch]);

  const getTournamentRulesData = () => {
    setLoader(true);
    dispatch(getTournamentRules({})).then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (simpleValidator.current.allValid()) {
      ...
      dispatch(createTournamentRules(payload)).then((res) => {
    ...
    <>
      ...
      <Paper sx={{ mb: 2 }} className="outer-box game-rules-section">
        ...
        {hideActionFunc("tournament") && (
          ...
          {
            checkRchEmpty() ?
            <button className={'btn mt_2'} type={"submit"} onClick={(e) => handleSubmit(e)}>Save Tournament Rules</button>
              :
            <button className={'btn mt_2'} type={"submit"} onClick={(e) => handleSubmit(e)}>Update Tournament Rules</button>
          }
          </div>
        )}
      </Paper>
      ...
      >
        <Modal
          ...
          handleOpenModal={handleOpenModal}
          modalIsOpen={modalDetails.modalIsOpen}
          redirectApiHandler={getTournamentRulesData}
        />
      </CommonModal>
    </>
  );
