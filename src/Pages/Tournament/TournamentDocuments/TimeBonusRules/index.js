...
import { createTimeBonusRules, getTimeBonusRules } from "Redux/Tournament/action";
...
const TimeBonusRules = () => {
  ...
  useEffect(() => {
    getTimeBonusRulesData();
  }, [dispatch]);

  const getTimeBonusRulesData = () => {
    setLoader(true);
    dispatch(getTimeBonusRules({})).then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (simpleValidator.current.allValid()) {
      ...
      dispatch(createTimeBonusRules(payload)).then((res) => {
    ...
    <>
      ...
      <Paper sx={{ mb: 2 }} className="outer-box game-rules-section">
        ...
        {hideActionFunc("master") && (
          ...
          {
            checkRchEmpty() ?
            <button className={'btn mt_2'} type={"submit"} onClick={(e) => handleSubmit(e)}>Save Time Bonus Rules</button>
              :
            <button className={'btn mt_2'} type={"submit"} onClick={(e) => handleSubmit(e)}>Update Time Bonus Rules</button>
          }
          </div>
        )}
      </Paper>
      ...
      >
        <Modal
          ...
          handleOpenModal={handleOpenModal}
          modalIsOpen={modalDetails.modalIsOpen}
          redirectApiHandler={getTimeBonusRulesData}
        />
      </CommonModal>
    </>
  );
};
export default TimeBonusRules;
