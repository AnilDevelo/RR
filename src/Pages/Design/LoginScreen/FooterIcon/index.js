...
const FooterIcon = () => {
  ...
  const getAllAvatarData = (startDate, endDate, search) => {
    ...
    setLoader(true);
    dispatch(getAllFooters(payload))
      .then((res) => {
      ...
      render: (row, i) => {
        return (
          <TableCell className={"game_icon_img"}>
            <img src={row?.footerIconImage || user} alt={""} />
          </TableCell>
        );
      },
    },
        ...
        return (
          <TableCell className={"role_field_id"}>
            <span
              ...
              onClick={() =>
                handleOpenModal("AddFooterPopup", { isEdit: true, row })
              }
            >
              Edit
            </span>
            <span
              ...
              onClick={() =>
                handleOpenModal("DeleteCommonModal", {
                  deleteListApiHandler: deleteFooterAvatarData({
                    loginScreenFooterId: row?._id
                  }),
  ...
  return (
    <Box>
      ...
      <Paper sx={{ mb: 2 }} className="outer-box">
        {hideActionFunc("master") && getAllAvatar?.list?.length !== 4 && (
          <div className={"d_flex_end"}>
            <button className={"btn"} onClick={(e) => handleOpenModal("AddFooterPopup")} > + Add Footer Icon </button>
          </div>
        )}
        <CustomTable
          headCells={columns}
