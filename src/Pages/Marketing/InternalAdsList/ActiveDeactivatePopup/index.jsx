...
import { activeDeactivateInternalAdsList } from "../../../../Redux/Master/action";
...
const ActiveDeactivatePopup = ({ modalValue, handleOpenModal, redirectApiHandler }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        setLoader(true);
        dispatch(activeDeactivateInternalAdsList(modalValue)).then(res => {
            if (res.data.success) {
                redirectApiHandler();
                setLoader(false);
        ...
        <Box sx={style} className={'user_popup_section'}>
            <div className='d_flex justify_content_center'>
                <Typography id="modal-modal-title" variant="h6" component="h2" className={"block-user-title"}>
                    {`Do you want to ${!modalValue?.isActive ? 'deactivate' : 'activate'} the Ads?`}
                </Typography>
            </div>
...
export default ActiveDeactivatePopup