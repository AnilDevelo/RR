...
import {activateReferAndEarnList} from "../../../../../Redux/Bonus/action";
...
const ActivateDeactivatePopup = ({ modalValue, handleOpenModal, redirectApiHandler }) => {
    ...
    const handleSubmit = (event) => {
        ...
        setLoader(true);
        dispatch(activateReferAndEarnList(modalValue)).then(res => {
            if (res?.data?.success) {
                redirectApiHandler();
                setLoader(false);
        ...
        <Box sx={style} className={'user_popup_section'}>
            <div className='d_flex justify_content_center'>
                <Typography id="modal-modal-title" variant="h6" component="h2" className={"block-user-title"}>
                    {modalValue?.isActive ? 'Do you want to activate the user?' : 'Do you want to deactivate the user?'}
                </Typography>
            </div>
