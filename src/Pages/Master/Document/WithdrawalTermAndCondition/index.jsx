...
import {
  createPrivacyPolicy,
  createWithdrawalTermAndCondition,
  getPrivacyPolicyList,
  getWithdrawalTermAndConditionList,
...
const WithdrawalTermAndCondition = () => {
  ...
  useEffect(() => {
    getWithdrawalTermAndConditionData();
  }, [dispatch]);

  const getWithdrawalTermAndConditionData = () => {
    setLoader(true);
    dispatch(getWithdrawalTermAndConditionList({})).then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (simpleValidator.current.allValid()) {
      ...
      dispatch(createWithdrawalTermAndCondition(payload)).then((res) => {
  ...
  const checkRchEmpty = (
    html = rowData?.description ? formData?.description : ""
  ) => {
  ...
  const handleOpenModal = (type, data) => {
    switch (type) {
      case "UpdateWithdrawalTermAndCondition":
      case "CommonPop": {
        setModalDetails({
          ...modalDetails,
          modalValue: data,
    ...
    <>
      <Paper sx={{ mb: 2 }} className="outer-box game-rules-section">
        ...
        {hideActionFunc("master") && (
          ...
          {
            checkRchEmpty() ?
            <button className={'btn mt_2'} type={"submit"} onClick={(e) => handleSubmit(e)}>Save Withdrawal Term & Condition</button>
              :
            <button className={'btn mt_2'} type={"submit"} onClick={(e) => handleSubmit(e)}>Update Withdrawal Term & Condition</button>
          }
          </div>
        )}
      </Paper>
      ...
      >
        <Modal
          ...
          handleOpenModal={handleOpenModal}
          modalIsOpen={modalDetails.modalIsOpen}
          redirectApiHandler={getWithdrawalTermAndConditionData}
        />
      </CommonModal>
    </>
  );
};
export default WithdrawalTermAndCondition;
