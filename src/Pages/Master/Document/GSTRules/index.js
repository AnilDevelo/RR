...
const GSTRules = () => {
  ...
  const getGSTRulesData = () => {
    setLoader(true);
    dispatch(getGSTRulesList({})).then((res) => {
      setLoader(false);
  ...
  const handleSubmit = (e) => {
    ...
    if (simpleValidator.current.allValid()) {
      ...
      dispatch(createGSTRules(payload)).then((res) => {
        if (res.data.success) {
    ...
    <>
      <Paper sx={{ mb: 2 }} className="outer-box game-rules-section">
        ...
        {hideActionFunc("master") && (
          ...
          {
            checkRchEmpty() ?
            <button className={'btn mt_2'} type={"submit"} onClick={(e) => handleSubmit(e)}>Save GST Rules</button>
              :
            <button className={'btn mt_2'} type={"submit"} onClick={(e) => handleSubmit(e)}>Update GST Rules</button>
          }
          </div>
        )}
      </Paper>
      ...
      >
        <Modal
          ...
          handleOpenModal={handleOpenModal}
          modalIsOpen={modalDetails.modalIsOpen}
          redirectApiHandler={getGSTRulesData}
        />
      </CommonModal>
    </>
  );
