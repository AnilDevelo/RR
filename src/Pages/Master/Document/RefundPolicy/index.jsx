...
const RefundPolicy = () => {
  ...
  const getRefundPolicyData = () => {
    setLoader(true);
    dispatch(getRefundPolicyList({})).then((res) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (simpleValidator.current.allValid()) {
      ...
      dispatch(createRefundPolicy(payload)).then((res) => {
        if (res.data.success) {
    ...
    <>
      ...
      <Paper sx={{ mb: 2 }} className="outer-box game-rules-section">
        ...
        {hideActionFunc("master") && (
          ...
          {
            checkRchEmpty() ?
            <button className={'btn mt_2'} type={"submit"} onClick={(e) => handleSubmit(e)}>Save Refund Policy</button>
              :
            <button className={'btn mt_2'} type={"submit"} onClick={(e) => handleSubmit(e)}>Update Refund Policy</button>
          }
          </div>
        )}
      </Paper>
      ...
      >
        <Modal
          ...
          handleOpenModal={handleOpenModal}
          modalIsOpen={modalDetails.modalIsOpen}
          redirectApiHandler={getRefundPolicyData}
        />
      </CommonModal>
    </>
  );
