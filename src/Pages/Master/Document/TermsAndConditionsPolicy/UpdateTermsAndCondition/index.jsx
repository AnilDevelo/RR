...
import { createTermsAndCondition } from "../../../../../Redux/Documentation/action";
...
const UpdateTermsAndCondition = ({ modalValue, handleOpenModal, redirectApiHandler }) => {
    const dispatch = useDispatch();
    const [loader, setLoader] = useState(false);
    const handleSubmit = (event) => {
        event.preventDefault();
        setLoader(true)
        dispatch(createTermsAndCondition(modalValue)).then((res) => {
            if (res.data.success) {
                redirectApiHandler();
                setLoader(false)
        ...
        <Box sx={style} className={'user_popup_section'}>
            <div className='d_flex justify_content_center'>
                <Typography id="modal-modal-title" variant="h6" component="h2" className={"block-user-title"}>
                    Do you want to change Terms & Conditions?
                </Typography>
            </div>
