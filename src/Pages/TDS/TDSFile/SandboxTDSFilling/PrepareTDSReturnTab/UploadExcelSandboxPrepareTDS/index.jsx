...
const UploadExcelSandboxPrepareTDS = ({modalValue, handleOpenModal, redirectApiHandler}) => {
    ...
    const [formData,setFormData] = useState({
        prepareTDSReturnId: modalValue?._id,
        preparedTDSReturnXlsxFile:'',
    });
    const simpleValidator = useRef(new SimpleReactValidator({
        validators: {
            ...
            preparedTDSReturnXlsxFile: {
                message: "Please enter valid .xlsx,.xls,.xlsm File.",
                rule: (val, params, validator) => {
                    return val.type === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' || validator.helpers.testRegex(val,/\.(xlsx|xls|xlsm)$/i) || val.type === 'application/vnd.ms-excel'
                },
                required: true
            },
        }
    ...
    const handleSubmit = (e) => {
        ...
        if (simpleValidator.current.allValid()) {
            ...
            setLoader(true)
            dispatch(uploadSandboxPrepareTDSReturnExcelFile(jsonToFormData(payload))).then(res => {
                if (res.data.success) {
                    setLoader(false)
                    redirectApiHandler();
    ...
    const handleChange = (e) => {
        ...
        setFormData({
            ...formData,
            preparedTDSReturnXlsxFile : e.target.files[0]
        })
    }
        ...
        <Box sx={style} className={'modal_main_popup sandbox_upload_file_details'}>
            ...
            <form className={'manage_game_builds'} onSubmit={(e) => handleSubmit(e)}>
                <div className={'user_kyc_section_filed_document'}>
                    <label>Upload Excel File <span className={'validation-star'}>*</span></label>
                        ...
                        <div className="u_flex u_align-center">
                            <div className="u_file-attachment add ">
                                ...
                                <div className="u_file-attachment-label">
                                    <input type="file" title=""  name={'preparedTDSReturnXlsxFile'} id="upload" autoComplete="off" onChange={(e)=>handleChange(e)} />
                                </div>
                            </div>
                        </div>
                    </div>
                    {simpleValidator.current.message("excelFile", formData?.preparedTDSReturnXlsxFile, 'required|preparedTDSReturnXlsxFile')}
                </div>
...
export default UploadExcelSandboxPrepareTDS