...
import {
    uploadSandboxEFileTDSReturnCSIFile,
...
const UploadCSIFileTDSReturn = ({modalValue, handleOpenModal, redirectApiHandler}) => {
    ...
    const simpleValidator = useRef(new SimpleReactValidator({
        validators: {
            ...
            eFileTDSReturnTXTFile: {
                message: "Please enter valid .csi File.",
                rule: (val, params, validator) => {
                    return validator.helpers.testRegex(val,/\.(csi)$/i)
                },
                required: true
            },
        }
    ...
    const handleSubmit = (e) => {
        ...
        if (simpleValidator.current.allValid()) {
            ...
            setLoader(true)
            dispatch(uploadSandboxEFileTDSReturnCSIFile(jsonToFormData(payload))).then(res => {
                if (res.data.success) {
                    setLoader(false)
                    redirectApiHandler();
        ...
        <Box sx={style} className={'modal_main_popup sandbox_upload_file_details'}>
            ...
            <form className={'manage_game_builds'} onSubmit={(e) => handleSubmit(e)}>
                <div className={'user_kyc_section_filed_document'}>
                    ...
                    {simpleValidator.current.message("CSIFile", formData?.eFileTDSReturnTXTFile, 'required|eFileTDSReturnTXTFile')}
                </div>
...
export default UploadCSIFileTDSReturn