...
import { userBlockUnblockListData } from "../../../../Redux/user/action";
...
const UnblockUserPopup = ({ modalValue, handleOpenModal, redirectApiHandler }) => {
    const dispatch = useDispatch();
    const [loader, setLoader] = useState(false);
    const handleSubmit = (event) => {
        event.preventDefault();
        setLoader(true)
        dispatch(userBlockUnblockListData(modalValue)).then(res => {
            if (res.data.success) {
                redirectApiHandler();
                setLoader(false)
