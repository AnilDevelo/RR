...
const RejectUserKYCRequest = ({ modalValue, handleOpenModal, redirectApiHandler }) => {
    ...
    const handleSubmit = (event) => {
        ...
        if (simpleValidator.current.allValid()) {
            ...
            dispatch(ApproveRejectUserKYCListRequest(payload)).then(res => {
                if (res?.data?.success) {
                    redirectApiHandler();
                    setLoader(false);
        ...
        <Box sx={style} className={'user_popup_section'}>
            <div className='d_flex justify_content_center'>
                <Typography id="modal-modal-title" variant="h6" component="h2" className={"block-user-title"}>
                    Do you want to reject the user KYC request?
                </Typography>
            </div>
...
export default RejectUserKYCRequest