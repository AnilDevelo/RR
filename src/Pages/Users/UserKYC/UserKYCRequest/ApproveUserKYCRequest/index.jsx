...
import { ApproveRejectUserKYCListRequest } from "../../../../../Redux/user/action";
...
const ApproveUserKYCRequest = ({ modalValue, handleOpenModal, redirectApiHandler }) => {
    const [loader, setLoader] = useState(false);
    const dispatch = useDispatch();
    const handleSubmit = (event) => {
        event.preventDefault();
        setLoader(true);
        dispatch(ApproveRejectUserKYCListRequest(modalValue)).then(res => {
            if (res.data.success) {
                redirectApiHandler();
                setLoader(false);
        ...
        <Box sx={style} className={'user_popup_section'}>
            <div className='d_flex justify_content_center'>
                <Typography id="modal-modal-title" variant="h6" component="h2" className={"block-user-title"}>
                    Do you want to approve user KYC request?
                </Typography>
            </div>
...
export default ApproveUserKYCRequest