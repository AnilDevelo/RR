...
import { phoneNumberUpdateApproveRejectRequest} from "../../../../Redux/user/action";
...
const RejectUserMobileNumber = ({ modalValue, handleOpenModal, redirectApiHandler }) => {
    ...
    const handleSubmit = (event) => {
        ...
        if (simpleValidator.current.allValid()) {
            setLoader(true);
            let payload = {
                phoneNumberUpdateRequestId: modalValue?.id,
                isApprove: false,
                rejectReason: formData
            }
            dispatch(phoneNumberUpdateApproveRejectRequest(payload)).then(res => {
                if (res?.data?.success) {
                    redirectApiHandler();
                    setLoader(false);
        ...
        <Box sx={style} className={'user_popup_section'}>
            <div className='d_flex justify_content_center'>
                <Typography id="modal-modal-title" variant="h6" component="h2" className={"block-user-title"}>
                    Do you want to reject the user phone number?
                </Typography>
            </div>
...
export default RejectUserMobileNumber;