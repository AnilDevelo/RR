...
import {deletePopularGame, updatePopularGame} from "../../../../Redux/popularGames/action";
...
const DeletePopularGame = ({ modalValue, handleOpenModal, redirectApiHandler }) => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        let temp = modalValue?.List.filter(item=>item?._id !== modalValue?.rowID);
        let payload = {
            isDefault:false,
            popularGameId:modalValue?.gameId,
            // gameId: temp?.map(item=> item?._id),
        };

        dispatch(deletePopularGame(payload)).then(res=>{
            if (res.data.success) {
                setLoader(false);
                redirectApiHandler();
...
export default DeletePopularGame;