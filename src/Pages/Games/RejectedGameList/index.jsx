...
const RejectedGameList = () => {
    const dispatch = useDispatch();
    const navigate = useNavigate();
    const [loader, setLoader] = useState(false);
    const gameList = useSelector(state => state.gameReducer?.gameAll);
    const [filterData, setFilterData] = useState({ startDate: null, endDate: null, statusValue: "All Days", exportFile: false, csvDownload: false, search: "", filterClose: false, exportFileName: 'Export File', platformName: 'All Platform' })
    ...
    let columns = [
        ...
        },
        {
            ...
            type: 'custom',
            render: (row) => {
                return <TableCell >{dotGenerator(row?.rejectReason, handleOpenModal, 'Game Description')}</TableCell>
            }
        },
        {
            id: 'createdAt',
        ...
        ActionFunction('game', {
            ...
            render: (row) => {
                return <TableCell className={'role_field_id'}>
                    <span className='edit_btn edit-btn-action u_border' onClick={() => handleOpenModal('ApprovedPendingPopup', { gameId: row?._id, isApprove: true })}>Approve</span>
                    <span className='edit_btn edit-btn-action prTab' onClick={() => handleOpenModal('ArchivedGamePopup', { gameId: row?._id, isArchive: true })}>Archive</span>
                </TableCell>
            }
        })
    ];
    ...
    useEffect(() => {
        // if (filterData.startDate && filterData.endDate || filterData?.statusValue === 'All Days') {
            allRejectedGameListFetch(filterData.startDate, filterData.endDate,filterData.search);
        // }
    ...
    const allRejectedGameListFetch = (startDate, endDate, search) => {
        setLoader(true)
        let payload = {
    ...
    useEffect(() => {
        if (filterData.startDate && filterData.endDate && filterData?.statusValue === 'Custom') {
            ...
            allRejectedGameListFetch(filterData.startDate, filterData.endDate)
        }
        ...
        <Box>
            ...
            <Paper sx={{ mb: 2 }} className="outer-box">
                <MainCommonFilter
                    ...
                    searchApiHandler={allRejectedGameListFetch}
                    pagination={pagination}
                    setPagination={setPagination}
                    plateFormOption={['All Platform', 'Ios', 'Android', 'Cross-Platform']}
                    addPropsFilter={{ isGameList: true, isPending: true, userPayment: gameList?.list?.length <= 0 }}
...
export default RejectedGameList;