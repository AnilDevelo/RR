...
const UpdateLeaderboardRules =  ({ modalValue, handleOpenModal, redirectApiHandler }) => {
    const { id } = useParams();
    ...
    const handleSubmit = (event) => {
        ...
        let payload = {
            gameId:id,
            leaderboardRules:modalValue?.description
        }
        ...
        <Box sx={style} className={'user_popup_section'}>
            <div className='d_flex justify_content_center'>
                <Typography id="modal-modal-title" variant="h6" component="h2" className={"block-user-title"}>
                    Do you want to change Leaderboard rules?
                </Typography>
            </div>
...
export default UpdateLeaderboardRules